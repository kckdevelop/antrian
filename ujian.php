...
.modal-backdrop {
  z-index: 9995 !important;
    ...
    header{padding:1rem;background:#0ea5a4;color:#fff}
    main{flex:1;padding:1.25rem;overflow:auto}

    /* overlay ketika tidak fullscreen / terdeteksi keluar */
    .fs-overlay{
      position:fixed;inset:0;display:flex;align-items:center;justify-content:center;
      background:linear-gradient(rgba(0,0,0,0.6),rgba(0,0,0,0.6));color:#fff;z-index:9999;
      text-align:center;padding:2rem;box-sizing:border-box;transition:opacity .2s ease;
    }
    .fs-overlay.hidden{display:none}
    .panel{background:rgba(255,255,255,0.06);padding:1.5rem;border-radius:10px;max-width:720px}
    button.primary{background:#06b6d4;border:0;padding:.6rem 1rem;border-radius:6px;color:#042;cursor:pointer}
    .note{font-size:.9rem;margin-top:.75rem;opacity:.9}

    /* gaya notifikasi kecil */
    .toast{position:fixed;right:1rem;bottom:1rem;background:#111;color:#fff;padding:.6rem .9rem;border-radius:8px;box-shadow:0 6px 18px rgba(0,0,0,.4)}
  </style> -->
...
<!-- <div id="fsOverlay" class="fs-overlay hidden" role="dialog" aria-modal="true">
    <div class="panel">
      <h2>Mode Fullscreen Diperlukan</h2>
      <p>Untuk menjaga integritas ujian, Anda harus memasuki mode fullscreen. Klik tombol di bawah ini untuk masuk fullscreen. Jika Anda keluar fullscreen berulang kali, ujian dapat dikunci atau dilaporkan.</p>
      <div style="margin-top:1rem;display:flex;gap:.6rem;justify-content:center">
        <button id="enterFs" class="primary">Masuk Fullscreen</button>
      <p class="note">Catatan: Beberapa browser atau perangkat mungkin meminta izin tambahan. Jika tombol tidak bekerja, instruksikan peserta memencet F11 (desktop) atau gunakan menu browser untuk ke fullscreen.</p>
    </div>
  </div>

  <div id="toast" class="toast" style="display:none">Anda meninggalkan fullscreen</div> -->
...
<!-- <script>
  (function(){
    ...
    const MAX_FS_EXIT = 3; // jumlah keluar fullscreen yang diizinkan sebelum tindakan
    const LOCK_ON_EXCEED = true; // jika true, kunci ujian saat melewati batas
    const fsOverlay = document.getElementById('fsOverlay');
    const enterFsBtn = document.getElementById('enterFs');
    const toast = document.getElementById('toast');

    let exitCount = 0;
    let locked = false; // jika locked true, hentikan ujian

    function showOverlay(){ fsOverlay.classList.remove('hidden'); }
    function hideOverlay(){ fsOverlay.classList.add('hidden'); }
    ...
    // Minta browser masuk fullscreen pada elemen <html>
    async function requestFullscreen(){
      try{
        ...
        console.warn('Fullscreen request gagal:', e);
      }
    }

    // Keluar fullscreen (opsional)
    ...
    // Saat fullscreen berubah
    document.addEventListener('fullscreenchange', ()=>{
      const isFs = !!document.fullscreenElement;
      console.log('fullscreenchange ->', isFs);
      if(isFs){
        hideOverlay();
        showToast('Anda berada di mode fullscreen', 1200);
      } else {
        // pengguna keluar fullscreen
        exitCount++;
        showOverlay();
        showToast(`Anda meninggalkan fullscreen (${exitCount}/${MAX_FS_EXIT})`, 3000);

        // jika melewati batas tindakan
        if(exitCount >= MAX_FS_EXIT && LOCK_ON_EXCEED && !locked){
          locked = true;
          handleLockExam();
        }
      }
    ...
    // Deteksi pergantian tab / minimize
    document.addEventListener('visibilitychange', ()=>{
      if(document.visibilityState !== 'visible'){
        // user berpindah tab atau minimize
        exitCount++;
        showOverlay();
        showToast('Terjadi pergantian tab / aplikasi', 2500);
        if(exitCount >= MAX_FS_EXIT && LOCK_ON_EXCEED && !locked) handleLockExam();
      }
    });

    // Beberapa browser akan blur saat berpindah aplikasi
    window.addEventListener('blur', ()=>{
      // catat tapi jangan spam: hanya jika bukan locked dan masih fullscreen keluar monitoring
      if(!locked && document.visibilityState === 'visible' && !document.fullscreenElement){
        // kemungkinan user alt-tab atau membuka layar mengambang
        exitCount++;
        showOverlay();
        showToast('Fokus aplikasi hilang', 2200);
        if(exitCount >= MAX_FS_EXIT && LOCK_ON_EXCEED && !locked) handleLockExam();
      }
    });

    // Tombol untuk meminta masuk fullscreen
    enterFsBtn.addEventListener('click', async ()=>{
      await requestFullscreen();
    });

    // Cegah klik kanan dan seleksi untuk mengurangi copy-paste (opsional)
    document.addEventListener('contextmenu', e => { e.preventDefault(); });
    document.addEventListener('copy', e => { /* e.preventDefault(); */ });

    // Cegah kombinasi tombol umum (tidak sepenuhnya aman karena browser/OS) 
    document.addEventListener('keydown', e =>{
      // F11, Ctrl/Command+T/N/W, Ctrl+Shift+I, Ctrl+Shift+J, Ctrl+U
      if(e.key === 'F11' || (e.ctrlKey && (e.key==='t' || e.key==='n' || e.key==='w' || e.key==='Shift')) ){
        e.preventDefault();
      }
      // Developer tools common
      const blocked = (e.ctrlKey && e.shiftKey && (e.key==='I' || e.key==='J')) || (e.ctrlKey && e.key==='U');
      if(blocked) e.preventDefault();
    });

    // Fungsi untuk mengunci ujian (mis. submit otomatis, laporkan, atau blokir interaksi)
    function handleLockExam(){
      // Contoh: tampilkan pesan dan nonaktifkan main content
      const app = document.getElementById('app');
      app.innerHTML = `\n        <div style="padding:2rem;text-align:center">\n          <h2>Ujian Dikunci</h2>\n          <p>Anda telah meninggalkan mode fullscreen terlalu sering. Silakan hubungi pengawas untuk melanjutkan.</p>\n        </div>`;
      // Di sistem nyata: kirim event ke server via fetch untuk logging
      try{
        navigator.sendBeacon && navigator.sendBeacon('/api/ujian/log-exit',{exits:exitCount});
      }catch(e){console.warn('sendBeacon gagal',e)}
    }

    // Saat halaman dimuat, minta fullscreen. Jika browser menolak, tampilkan overlay
    window.addEventListener('load', async ()=>{
      // beri waktu singkat sebelum minta fullscreen agar gesture user lebih dapat diterima di mobile
      setTimeout(async ()=>{
        try{
          await requestFullscreen();
    ...
    // Untuk pengembang: fungsi reset count (bisa dipanggil dari konsol)
    window._examHelpers = {
      resetExitCount(){ exitCount = 0; locked=false; hideOverlay(); console.log('exitCount reset'); },
      getExitCount(){ return exitCount; }
    };
